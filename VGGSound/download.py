...
def meta_data_clean_df_exist_file(data_dir, df):
    ...
    for (root, dirs,files) in os.walk(data_dir, topdown=True):
        pattern = r'(\d+)_(\d+)_out.mp4'
        for filename in files:
            print(filename)
            if filename.find('.mp4') != -1 and filename[0] == 'v':
...
def download_and_process(data_dir, df_row):
    ...
    output_file_name = "v{}_{}_{}_out.mp4".format(video_id, start_time, end_time)
    
    ffmpeg_extract_segment(inputfile, os.path.join(category_dir, output_file_name), start_time, end_time)
    print(os.path.join(category_dir, output_file_name))
    os.remove(inputfile)
    return None, df_row['video_id']

def downloadVGGSound(csv_path, err_csv_path, data_dir):
...
if __name__ == "__main__": # E:\DATA\VGGSoundCode\VGGSound\split_downfiles\chunk_000.csv
    csv_path = 'E:/DATA/VGGSoundCode/VGGSound/vggsound.csv'
    err_name = os.path.basename(csv_path)
    err_csv_path = f"error_{err_name}"
    data_dir = 'E:/DATA/VGGSound/'
    downloadVGGSound(csv_path, err_csv_path, data_dir)